...
int ABMConteo::Add(string idLista, int idDistrito, Eleccion* eleccion){
	...
	KeyInt key = idConteo;
	string str = idLista.append("|").append(Helper::IntToString(idDistrito)).append("|").append(Helper::IntToString(eleccion->GetId()));
...
void ABMConteo::AddVoto(int idConteo, Votante* votante){
	...
	if(c != NULL){
		...
		//Esta es la clave de la eleccion dentro de conteo
		str.append("|").append(Helper::IntToString(c->GetEleccion()->GetId()));	//agrego id eleccion
...
void ABMConteo::NotifyVotante(Votante* votante, Eleccion* eleccion){
	...
	votante->AgregarEleccion(eleccion->GetId());
...
Conteo* ABMConteo::GetConteo(int idConteo){
	...
	cout << "pepe1" << endl;
	LeafNode* node = this->bplusTree->find(idConteo);
	cout << "chan" << endl;
	Element* elemento = node->getElement(idConteo);
...
vector<Conteo> ABMConteo::GetConteoByEleccion(Eleccion eleccion){
	...
	vector<Conteo> conteos;
	vector<Key> ids = this->indexByEleccion->GetIds(Helper::IntToString(eleccion.GetId()));
